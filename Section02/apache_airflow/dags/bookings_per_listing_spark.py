...
def main():
    ...
    listings = spark.read.csv(args.listings_file, header=True, inferSchema=True, sep=",", quote='"', escape='"', multiLine=True, mode="PERMISSIVE")
    bookings = spark.read.csv(args.bookings_file, header=True, inferSchema=True,)

    aggregated = listings.join(bookings, listings["id"] == bookings["listing_id"], how="inner").groupBy("listing_id", "name", "price").agg(count("booking_id").alias("booking_count"))
